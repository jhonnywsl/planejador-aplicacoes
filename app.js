...
function buildPlainText(plan) {
  ...
  plan.applications.forEach((app) => {
    ...
    lines.push(`${formatDate(app.date)} (${weekday(app.date)}): ${doseText}. Total ${volume(app.totalMl)} ml. Local: ${app.site}.`);
  });
...
function buildTaskText(plan) {
  return plan.applications.map((app) => {
    const doseText = app.doses.map((dose) => `${dose.name} ${volume(dose.mlPerApplication)} ml`).join(" + ");
    return `${formatDate(app.date)} - Aplicação: ${doseText}; total ${volume(app.totalMl)} ml; local ${app.site}`;
  }).join("\n");
}
...
function buildIcs(plan) {
  ...
  const events = plan.applications.map((app) => {
    ...
    const desc = [
      ...app.doses.map((dose) => `${dose.name}: ${volume(dose.mlPerApplication)} ml (${number(dose.mgPerApplication, 1)} mg)`),
