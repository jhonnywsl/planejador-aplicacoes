...
function renderPlan(plan) {
  ...
  metricsEl.innerHTML = [
    ...
    metric("Volume por aplicação", `${number(plan.applications[0]?.totalMl || 0)} ml`, "soma das substâncias"),
    metric("Volume total", `${number(totalMl)} ml`, `${number(totalMg, 1)} mg no período`)
  ].join("");
  ...
  scheduleBody.innerHTML = plan.applications.map((app) => `
    <tr>
      ...
      <td>${app.doses.map((dose) => `${escapeHtml(dose.name)}: ${number(dose.mgPerApplication, 1)} mg / ${number(dose.mlPerApplication)} ml`).join("<br>")}</td>
      <td>${number(app.totalMl)} ml</td>
    </tr>
  `).join("");
...
function buildPlainText(plan) {
  ...
  plan.doses.forEach((dose) => {
    lines.push(`- ${dose.name}: ${number(dose.mgPerApplication, 1)} mg = ${number(dose.mlPerApplication)} ml`);
  });

  lines.push(`- Total: ${number(plan.applications[0]?.totalMl || 0)} ml`);
  lines.push("");
  lines.push("Calendário:");

  plan.applications.forEach((app) => {
    const doseText = app.doses.map((dose) => `${dose.name} ${number(dose.mlPerApplication)} ml`).join(" + ");
    lines.push(`${formatDate(app.date)} (${weekday(app.date)}): ${doseText}. Total ${number(app.totalMl)} ml. Local: ${app.site}.`);
  });
...
function buildTaskText(plan) {
  return plan.applications.map((app) => {
    const doseText = app.doses.map((dose) => `${dose.name} ${number(dose.mlPerApplication)} ml`).join(" + ");
    return `${formatDate(app.date)} - Aplicação: ${doseText}; total ${number(app.totalMl)} ml; local ${app.site}`;
  }).join("\n");
}
...
function buildIcs(plan) {
  ...
  const events = plan.applications.map((app) => {
    ...
    const desc = [
      ...app.doses.map((dose) => `${dose.name}: ${number(dose.mlPerApplication)} ml (${number(dose.mgPerApplication, 1)} mg)`),
      `Total: ${number(app.totalMl)} ml`,
      `Local: ${app.site}`,
...
document.querySelector("#icsBtn").addEventListener("click", () => {
  const plan = requirePlan();
  download("planejamento-aplicacoes.ics", buildIcs(plan), "text/calendar;charset=utf-8");
});

document.querySelector("#tasksBtn").addEventListener("click", () => {
  const plan = requirePlan();
  download("tarefas-aplicacoes.txt", buildTaskText(plan), "text/plain;charset=utf-8");
});
